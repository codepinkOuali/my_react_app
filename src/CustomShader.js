  ...
  constructor() {
    super({
      ...
      fragmentShader: /* glsl */ `
        ...
        void main() {
          // Create swirling effect using Perlin noise
          vec2 uv = vUv * 2.0 - 1.0; // Center UV coordinates
          float angle = atan(uv.y, uv.x); // Calculate angle from center
          float radius = length(uv); // Calculate distance from center

          // Apply Perlin noise to create organic swirls
          float noise = cnoise3(vec3(uv * 3.0, time * 0.5));
          float swirl = sin(angle * 5.0 + time * 2.0 + noise * 2.0) * 0.5 + 0.5;

          // Mix colors based on swirl and radius
          vec3 color = mix(colorStart, colorEnd, swirl * (1.0 - radius));

          // Add a radial gradient for depth
...
CustomShader.key = guid.generate();
extend({ CustomShader });
